...
const CartContents = ({ cart, userId, guestId }) => {
  ...
  const handleAddToCart = async ({ productId, delta, quantity, size, color }) => {
    ...
    setLoading(true);
    try {
      await dispatch(updateCartItemQuantity({ productId, quantity: newQuantity, guestId, userId, size, color }));
  ...
  const handleRemoveFromCart = async ({ productId, size, color }) => {
    setLoading(true);
    try {
      await dispatch(removeFromCart({ productId, guestId, userId, size, color })).unwrap();
      dispatch(fetchCart({ userId, guestId }));
  ...
  useEffect(() => {
    // Ensure cart is fetched when user or guest ID changes
    dispatch(fetchCart({ userId, guestId }));
  }, [dispatch, userId, guestId]);

  return (
    <div>
      {/* Conditionally render cart contents if available */}
